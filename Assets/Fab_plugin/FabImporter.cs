                    ...
                    try {
                        ...
                        for (int i = 0; i < payloads.Count; ++i)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    string native_file_path = FabUtilities.MapWindowsPathToLinux((string)native_files[j]);
                            ...
                            if(models != null)
                            {
                                for(int j = 0 ; j < models.Count ; j++)
                                {
                                    try
                                    {
                                        ...
                                        string model_file = FabUtilities.MapWindowsPathToLinux((string)model_payload["file"]);
                                        ...
                                        if (Array.IndexOf(gltfExtensions, extension) > -1)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                gltfDirectory = FabUtilities.MapWindowsPathToLinux(gltfDirectory);
                                        ...
                                        else if (Array.IndexOf(usdExtensions, extension) > -1)
                                        {
                                            ...
                                            {
                                                string usdDir = Path.GetDirectoryName(model_file);
                                                usdDir = FabUtilities.MapWindowsPathToLinux(usdDir);
                                                FabUtilities.CopyDirectory(usdDir, rootPath);
                                            }
                            ...
                            if(additional_textures != null)
                            {
                                if(additional_textures.Count>0)
                                {
                                    ...
                                    for(int j = 0 ; j < additional_textures.Count ; j++)
                                    {
                                        try
                                        {
                                            string srcTex = FabUtilities.MapWindowsPathToLinux((string)additional_textures[i]);
                                            string texturePath = Path.Combine(additionalTexturesPath, Path.GetFileName(srcTex));
                                            FabUtilities.CopyFileToProject(srcTex, texturePath);
